    ...
    with open(file_path, "w") as file:
        for _, row in df.iterrows():
            ...
            for idx, value in enumerate(row):
                ...
                elif idx == 1:  # Name field
                    formatted_value = str(value).upper().ljust(col_widths[idx], ' ')
                elif idx == 2:  # Employee ID field
                    formatted_value = "N" + str(value[1:]).ljust(col_widths[idx] - 1, ' ')
                else:
