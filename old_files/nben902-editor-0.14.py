...
class EditableTable(tk.Frame):
    ...
    def add_row(self, record=None):
        ...
        duplicate_button = tk.Button(self, text="Duplicate", command=lambda row=len(self.entries)-1: self.duplicate_row(row))
        duplicate_button.grid(row=len(self.entries), column=len(self.columns) + 1)
    ...
    def delete_row(self, row):
        ...
        for i in range(row, len(self.entries)):
            ...
            self.duplicate_buttons[i].grid(row=i+1, column=len(self.columns) + 1)
